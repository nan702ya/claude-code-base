...
- **Medium 사이클 마지막 단계인 경우**: 코멘트 추가 후 `mcp__linear__save_issue`로 status → `done`, 이후 **브랜치 병합 및 정리** 수행
...
Medium 사이클의 마지막 단계(peer-review) 또는 Large 사이클에서 document 전 단계에서, 모든 검토가 통과되면 작업 브랜치를 main에 병합하고 삭제한다.
...
> **주의**: 조치 필요 항목이 있어 execute로 복귀하는 경우에는 병합하지 않는다.
